...
int _toInt(dynamic v) {
 ...
 TableTimerInfo? timerOf(String table) => tableTimers[table];
  ...
  static const Duration _startTimePreviousDayThreshold = Duration(hours: 6);
  ...
  DateTime? _resolveStartDateTime(String? hhmm, {DateTime? now}) {
    ...
    var start = DateTime(current.year, current.month, current.day, h, m);

    // 基本は当日扱い。
    // ただし、開始時刻が「現在よりかなり先」の場合は前日扱いに補正する（深夜跨ぎ対応）。
    if (start.isAfter(current) &&
        start.difference(current) > _startTimePreviousDayThreshold) {
  ...
  int _calcRemainingSecondsFromClock({
    ...
    final current = now ?? DateTime.now();
    final end = start.add(Duration(seconds: totalSeconds < 0 ? 0 : totalSeconds));
    final remaining = end.difference(current).inSeconds;
